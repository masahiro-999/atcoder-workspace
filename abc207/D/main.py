...
if N == 1:
    ...
    dist_01 = dist(*ab[0],*ab[1])
    target = create_set(0,1,ab)
    ans = NO
    for i,j in permutations(range(N),2):
        if dist_01 != dist(*cd[i],*cd[j]):
            continue
        result = create_set(i,j,cd)
        # print(i,j,result)
        if target == result:
            ans = YES
            break
