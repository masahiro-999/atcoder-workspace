...
accC = list(accumulate(C))
# print(accA)
# print(accB)
# print(accC)

ans = accA[min(M, len(A))]
p =1
for i,xc in enumerate(accC, start=1):
    np = p
    for j in range(p,min(xc,len(B))+1):
        if M-i-j <0:
            break
        np = j
        a = accA[min(M-i-j,len(A))]+accB[j]
        ans = max(ans,a)
    p = max(0,np-1)
