...
dp0 = [[0]*(3+1) for _ in range(C+1)]

for i in range(R):
    dp1 = [[0]*(3+1) for _ in range(C+1)]
    for j in range(C):
        v = t[i][j]
        upvalue = max(dp0[j+1])
        dp1[j+1][0] = max(upvalue, dp1[j][0])
        for k in range(3):
            dp1[j+1][k+1] = max(upvalue+v, dp1[j][k]+v, dp1[j][k+1])
    dp0 = dp1
# print(dp)
ans = max(dp0[C])
print(ans)
