...
for i in range(T):
    A = LII()
    P = LII()
    def get_cost(sm_n, sm_value, i):
        d = (3*sm_n-sm_value+(i-3-1))//(i-3)
        return P[i-1]*d
    ...
    c4 = get_cost(sm_n, sm_value, 4)
    c5 = get_cost(sm_n, sm_value, 5)
    c45 = get_cost(sm_n+1, sm_value+4,5)+P[4-1]
    # print(f'{c4=} {c5=}')
    min_cost = max(0,min(c4, c5, c45))
    print(min_cost)
