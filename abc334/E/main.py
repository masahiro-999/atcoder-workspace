...
for i,j in product(range(H),range(W)):
    ...
    if j+1<W and S[i][j+1]=="#":
        if uf.union(pos,pos+1):
            num_g -= 1
    if i+1<H and S[i+1][j]=="#":
        if uf.union(pos,pos+W):
            num_g -= 1
...
for i,j in product(range(H),range(W)):
    ...
    for di,dj in[(-1,0),(1,0),(0,-1),(0,1)]:
        ...
        if 0<=i+di<H and 0<=j+dj<W and S[i+di][j+dj] == "#":
            neighber_root.add(uf.find(pos+d_pos))
    ans += (num_g-(len(neighber_root)-1))*pow(cnt_red,-1,MOD)
    # ans += (num_g-(len(neighber_root)-1))
    ans %= MOD
