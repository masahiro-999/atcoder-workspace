...
XA,YA,XB,YB,XC,YC=LII()

def d(x1,y1,x2,y2):
    a = 0
    if x1==x2==XB and (y1<YB<y2 or y1>YB>y2):
        a = 2
    elif y1==y2==YB and (x1<XB<x2 or x1>XB>x2):
        a = 2
...
if XC==XB:
    ans = d(XA,YA,XB,YB-1)+YC-YB
elif YC==YB:
    ans = d(XA,YA,XB-1,YB)+XC-XB
else:
    ans1 = d(XA,YA,XB-1,YB)+XC-XB+2+YC-YB
    ans2 = d(XA,YA,XB,YB-1)+YC-YB+2+XC-XB
