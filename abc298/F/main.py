...
r_back = defaultdict(list)
rc_x = Counter()
for i,(r,c,x) in enumerate(rcx):
    r_back[r].append(i)
    rc_x[(r,c)]=x

c_max = [(sm,c) for c,sm in sum_c.items()]
c_max.sort(reverse=True)
sl = SortedList(sum_c.values())
ans = 0
for i in sum_r.keys():
    try_c =[]
    for j in r_back[i]:
