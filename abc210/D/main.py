...
def get_ans(H,W,C,A):
    min_table = [[0]*W for _ in range(H)]
    ans = inf
    for i,j in product(range(H), range(W)):
        n = inf
        if j > 0:
            n = min(n, min_table[i][j-1])
        if i > 0:
            n = min(n, min_table[i-1][j])
        min_table[i][j] = min(n, A[i][j]-C*(i+j))
        ans = min(ans, n+A[i][j]+C*(i+j))
    return ans

A1 = [a[::-1] for a in A]

ans = min(get_ans(H,W,C,A), get_ans(H,W,C,A1))
